            ...
            <nav className="relative z-10 w-full px-6 py-6 border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    <Link href={route('dashboard')} className="flex items-center space-x-3 group">
                        ...
                        <span className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-[#FF2D20] transition-colors">DrugTick Management</span>
                    </Link>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-slate-200 dark:border-slate-800">
                                        ...
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">User Information</th>
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Activity</th>
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Current Role</th>
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Member Since</th>
                                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 text-right">Assign New Role</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                                    {users.map((user) => (
                                        <tr key={user.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                                            <td className="px-6 py-4 text-sm font-medium text-slate-900 dark:text-white">#{user.id}</td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <select
                                                    ...
                                                    className="text-xs font-bold bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-lg focus:ring-[#FF2D20] disabled:opacity-50 transition-all"
                                                >
                                                    <option value="user">USER</option>
                                                    <option value="admin">ADMIN</option>
                                                </select>
                                                {user.id === auth.user.id && (
                                                    <div className="text-[10px] text-slate-400 mt-1 uppercase font-bold">Current Session</div>
                                                )}
                                            </td>
                                        </tr>
                                    ))}
            ...
            <footer className="relative z-10 w-full px-6 py-8 border-t border-slate-200 dark:border-slate-800">
                <div className="max-w-7xl mx-auto text-center">
                    <p className="text-sm text-slate-500 dark:text-slate-500">
                        &copy; {new Date().getFullYear()} DrugTick Management. All rights reserved.
                    </p>
                </div>
            </footer>
        </div>
