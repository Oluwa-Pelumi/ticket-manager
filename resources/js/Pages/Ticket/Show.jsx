                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        ...
                        <div className="space-y-8">
                            <div>
                                ...
                                <div className="p-8 rounded-[2.5rem] bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200 dark:border-slate-800 shadow-2xl">
                                    ...
                                    <div className="flex items-center gap-3 mb-6 group/id">
                                        <div className="text-lg md:text-xl text-slate-900 dark:text-white font-bold break-all">{ticket.id}</div>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleCopy(ticket.id); }}
                                            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-[#FF2D20] transition-all border border-transparent hover:border-[#FF2D20]/20"
                                            title="Copy ID"
                                        >
                                            {copiedId === ticket.id ? (
                                    ...
                                    <div className="text-xl text-slate-900 dark:text-white font-bold mb-6">{subjects.find(s => s.value == ticket.subject)?.name || ticket.subject}</div>

                                    <div className="text-[10px] font-black text-[#FF2D20] mb-2 uppercase tracking-[0.2em]">Priority</div>
                                    <div className="mb-6">
                                        <span className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider ${
                        ...
                        <div className="space-y-8">
                            <div>
                                ...
                                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar mb-6 p-6 rounded-[2.5rem] bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200 dark:border-slate-800 shadow-2xl">
                                    {ticket.comments?.length > 0 ? ticket.comments.map((comment, ci) => (
                                        <div key={ci} className={`flex flex-col ${comment.user_id === ticket.user_id || (!comment.user_id && !ticket.user_id) ? 'items-end' : 'items-start'}`}>
                                            <div className={`max-w-[85%] p-4 rounded-3xl ${comment.user_id === ticket.user_id || (!comment.user_id && !ticket.user_id) ? 'bg-[#FF2D20] text-white rounded-br-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white rounded-bl-sm'}`}>
                                                <div className="flex items-center space-x-2 mb-2">
                                                    <span className="text-[10px] font-black uppercase opacity-70">{comment.user?.name || 'Guest'}</span>
                                                ...
                                                {comment.images?.length > 0 && (
                                                    <div className="flex flex-wrap gap-2 mt-3">
                                                        {comment.images.map((cimg, cii) => (
                                                            <a key={cii} href={`/storage/${cimg}`} target="_blank" className="w-16 h-16 rounded-xl overflow-hidden border border-white/20">
                                    ...
                                    <form onSubmit={handleCommentSubmit} className="space-y-4">
                                        <div className="relative group/comment">
                                            <textarea
                                                ...
                                                rows="4"
                                                className="w-full px-6 py-5 rounded-[2rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-[#FF2D20] outline-none transition-all resize-none shadow-xl"
                                                required
                                            ></textarea>
                                            ...
                                            <div className="absolute bottom-4 right-4 flex items-center space-x-2">
                                                ...
                                                />
                                                <label htmlFor="comment-images" className="p-3 text-slate-400 hover:text-[#FF2D20] hover:bg-[#FF2D20]/10 rounded-2xl cursor-pointer transition-all bg-slate-50 dark:bg-slate-800">
                                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                                </label>
                                                <button
                                                    type="submit"
                                                    disabled={commentForm.processing || !commentForm.data.content.trim()}
                                                    className="p-3 bg-[#FF2D20] text-white rounded-2xl shadow-lg shadow-[#FF2D20]/20 hover:scale-110 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100"
                                                >
