    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div class="flex justify-end shrink-0">
                <button
                    x-data
                    @click="$dispatch('open-faq-modal', { faq: null })"
...
@section('title', 'FAQ Management')
        ...
        <div class="grid grid-cols-1 gap-6">
                    ...
                    <div class="flex justify-between items-start gap-6">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-4 mb-3">
                                <span class="px-3 py-1 rounded-full bg-teal-900/10 dark:bg-lime-500/10 text-teal-900 dark:text-lime-400 text-[10px] font-black uppercase tracking-widest border border-teal-900/20 dark:border-lime-500/20">
                                    Order: {{ $faq->order ?? 0 }}
                                </span>
                            </div>
            ...
            @empty
                <div class="fauna-panel p-20 text-center">
                    <div class="w-20 h-20 rounded-[2rem] bg-slate-100 dark:bg-[#18342f] flex items-center justify-center mx-auto mb-6">
                        <svg class="w-10 h-10 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                    </div>
                    <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-2 uppercase italic tracking-tighter">
                        No FAQs Found
                    </h3>
    ...
    <div
        x-data="{
            ...
            init() {
                window.addEventListener('open-faq-modal', (e) => {
    ...
    >
        <div
            class="w-full max-w-2xl fauna-panel p-6 sm:p-10 bg-white/95 dark:bg-[#102824]/95 border-lime-500/20 overflow-y-auto max-h-[90vh]"
            x-transition:enter="transition ease-out duration-300"
        ...
        >
            ...
            <div class="flex justify-between items-center mb-10">
                ...
                <button @click="close()" class="p-3 rounded-2xl bg-slate-100 dark:bg-[#18342f] text-slate-500 hover:text-rose-500 transition-colors">
            ...
            >
                ...
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-teal-900 dark:text-lime-400 uppercase tracking-[0.3em] block">Question</label>
                    <input
                        type="text"
                        name="question"
                        x-model="question"
                    ...
                    />
                    @error('question')
                        <p class="text-rose-500 text-[10px] font-black uppercase mt-2 tracking-widest">{{ $message }}</p>
                    @enderror
                </div>

                <div class="space-y-3">
                    <label class="text-[10px] font-black text-teal-900 dark:text-lime-400 uppercase tracking-[0.3em] block">Answer</label>
                    <textarea
                        name="answer"
                        x-model="answer"
                ...
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-teal-900 dark:text-lime-400 uppercase tracking-[0.3em] block">Display Order</label>
                    <input
                        type="number"
                        name="order"
                        x-model="order"
            ...
            <form
                method="POST"
                :action="`{{ url('admin/faqs') }}/${faq ? faq.id : ''}`"
                x-show="faq"
                class="space-y-8"
            >
                @csrf
