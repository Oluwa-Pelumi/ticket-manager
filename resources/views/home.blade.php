...
<x-app-layout :show-navbar="false">
    ...
    <div class="fauna-shell min-h-screen">
        ...
        <section class="relative overflow-hidden hero-gradient bg-teal-900 dark:bg-[#102824]">
            {{-- Gorgeous blurred background elements --}}
            <div class="absolute inset-0 z-0">
                <div
                    class="absolute -top-[20%] -left-[10%] w-[50%] h-[50%] rounded-full bg-emerald-300/20 dark:bg-emerald-900/10 blur-[120px]">
                </div>
                <div
                    class="absolute top-[20%] -right-[10%] w-[40%] h-[60%] rounded-full bg-green-400/15 dark:bg-emerald-900/10 blur-[120px]">
                </div>
                <div
                    class="absolute -bottom-[20%] left-[20%] w-[60%] h-[40%] rounded-full bg-emerald-200/15 dark:bg-emerald-900/10 blur-[120px]">
                </div>
                ...
                <nav class="py-6">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('home') }}" class="inline-flex items-center gap-2.5 sm:gap-3 text-white group shrink-0 min-w-0">
                            <img src="{{ asset('logo.svg') }}?v=4" alt="{{ $appName }} logo" class="w-10 h-10 sm:w-12 sm:h-12 drop-shadow-lg transition-transform group-hover:scale-105 shrink-0">
                            <div class="flex flex-col leading-none min-w-0">
                                <span class="text-xs sm:text-sm md:text-base lg:text-lg font-black tracking-tight text-rose-400 drop-shadow-md truncate">
                                    {{ $mainName }}
                                </span>
                                @if($subName)
                                    <span class="text-[9px] sm:text-[10px] lg:text-xs font-bold tracking-wider uppercase text-slate-200 drop-shadow-md truncate mt-0.5">
                                        {{ $subName }}
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="flex items-center gap-3 sm:gap-4">
                            @guest
                                <a href="{{ route('login') }}"
                                    class="block sm:hidden px-5 py-2.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 text-white text-xs sm:text-sm font-bold backdrop-blur-md transition-all hover:scale-105 active:scale-95 shadow-lg">Login</a>
                                <a href="{{ route('login') }}"
                                    class="hidden sm:block px-5 py-2.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 text-white text-xs sm:text-sm font-bold backdrop-blur-md transition-all hover:scale-105 active:scale-95 shadow-lg">Login</a>

                                <a href="{{ route('register') }}"
                                    class="hidden sm:block px-5 py-2.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 text-white text-xs sm:text-sm font-bold backdrop-blur-md transition-all hover:scale-105 active:scale-95 shadow-lg">Register</a>
                            @else
                                <a href="{{ route('dashboard') }}"
                                    class="px-5 py-2.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 text-white text-xs sm:text-sm font-bold backdrop-blur-md transition-all hover:scale-105 active:scale-95 shadow-lg">Dashboard</a>
                            @endauth
                            <button onclick="toggleTheme()"
                                class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-white transition-all backdrop-blur-md hover:scale-110 active:scale-95"
                ...
                <div class="container mx-auto px-4">
                    <div class="grid grid-cols-1 gap-8 text-center md:grid-cols-4">
                        @foreach ([['label' => 'Total Tickets', 'value' => $stats['totalTickets'] ?? 0], ['label' => 'Open Tickets', 'value' => $stats['openTickets'] ?? 0], ['label' => 'In Progress', 'value' => $stats['inProgressTickets'] ?? 0], ['label' => 'Resolved Tickets', 'value' => $stats['resolvedTickets'] ?? 0]] as $stat)
                            <div class="fauna-panel p-8 dark:bg-[#0b1f1c] dark:border-[#1d3a34]">
        ...
        <section class="py-20 bg-slate-50/50 dark:bg-[#060e0c] relative overflow-hidden">
            {{-- Decorative background elements --}}
            <div class="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-full pointer-events-none">
                <div
                    class="absolute top-20 right-10 w-96 h-96 bg-emerald-400/5 dark:bg-emerald-400/10 rounded-full blur-3xl">
                </div>
                <div
                    class="absolute bottom-10 left-10 w-72 h-72 bg-lime-400/5 dark:bg-lime-400/10 rounded-full blur-3xl">
                </div>
            </div>

            <div class="container mx-auto px-4 relative z-10">
                <div class="text-center max-w-3xl mx-auto mb-16">
                    <h2 class="text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight mb-6">How
                        ticketing works
                    </h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto mb-16 relative">
                    ...
                    <div
                        class="hidden lg:block absolute top-[2.5rem] left-[12.5%] right-[12.5%] h-0.5 bg-gradient-to-r from-emerald-100 via-emerald-200 to-emerald-100 dark:from-emerald-900/50 dark:via-emerald-800/50 dark:to-emerald-900/50 z-0">
                    </div>
                    ...
                    @foreach ($ticketingSteps as $index => $item)
                        <div class="group relative z-10">
                            {{-- Step card --}}
                            <div
                                class="h-full p-8 rounded-[2rem] bg-white dark:bg-[#0b1f1c] border border-emerald-100 dark:border-[#1d3a34] shadow-xl shadow-emerald-900/5 dark:shadow-none hover:-translate-y-2 hover:shadow-2xl hover:shadow-emerald-900/10 dark:hover:border-emerald-400/30 transition-all duration-300">

                                {{-- Step number badge --}}
                                <div
                                    class="w-16 h-16 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/30 flex items-center justify-center text-2xl font-black text-emerald-600 dark:text-emerald-400 mb-8 group-hover:scale-110 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/40 transition-all duration-300">
                                    {{ $item['step'] }}
                                </div>

                                <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-4">{{ $item['title'] }}
                                </h3>
                ...
                <div class="flex flex-wrap items-center justify-center gap-4 pt-8">
                    <a href="{{ route('submit-ticket') }}"
                        class="px-8 py-4 bg-white dark:bg-[#18342f] text-slate-700 dark:text-slate-300 border border-emerald-400 dark:border-[#1d3a34] rounded-[2rem] font-black text-sm tracking-widest uppercase hover:bg-slate-50 dark:hover:bg-[#0b1f1c] hover:text-emerald-950 dark:hover:text-white transition-all inline-flex items-center gap-3">
                        Create ticket
                    </a>
