...
/**
 * Tests for public and auth pages not covered by controller-specific test files:
 * home, check-status, ticket statuses polling, and the root redirect.
 */
class GeneralRoutesTest extends TestCase
{
    ...
    public function test_home_page_stats_reflect_ticket_counts(): void
    {
        ...
        Ticket::factory()->count(1)->create(['status' => 'in-progress']);
        ...
        $this->assertEquals(1, $stats['closedTickets']);
    ...
    {
        Faq::factory()->create(['order' => 2, 'question' => 'B']);
        Faq::factory()->create(['order' => 1, 'question' => 'A']);

        $response = $this->get(route('home'))->assertOk();
        $faqs     = $response->viewData('faqs');

        $this->assertEquals('A', $faqs[0]->question);
        $this->assertEquals('B', $faqs[1]->question);
    }
    ...
    /** @test */
    public function test_submit_ticket_page_is_accessible_by_guests(): void
    {
        $this->get(route('submit-ticket'))
            ->assertOk()
            ->assertViewIs('submit-ticket');
    }

    /** @test */
    public function test_submit_ticket_page_passes_categories_to_view(): void
    {
        Category::factory()->count(2)->create();

        $response = $this->get(route('submit-ticket'))->assertOk();
    ...
    {
        $this->getJson(route('tickets.statuses'))
            ->assertUnauthorized();
    }
    ...
    /** @test */
    public function test_support_gets_only_their_assigned_statuses(): void
    {
